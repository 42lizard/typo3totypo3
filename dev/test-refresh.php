...
function due(array $reference): void
{
    global $dest;
...
try {
    ...
    check($initial['status'] === 'resolved', 'Real HTTPS resolver establishes destination');
    $references[] = $ref = $initial['reference'];
    $store->record($ref, 'resolved', $initial['url']);
    check($worker->run()['processed'] === 0, 'Fresh destinations are not immediately refreshed');
    $alias = $client->resolve('self', ['https://old.invalid' . $slug . '?utm_source=x#section'])[0];
    check($alias['reference'] === $ref && $alias['url'] === $origin . $slug . '?utm_source=x#section', 'Explicit old origin resolves locally and preserves suffixes');
    $config['publicAliases'] = ['https://old.invalid' => 'https://unconfigured.invalid'];
    saveConfig();
    check($client->resolve('self', ['https://old.invalid' . $slug])[0]['status'] === 'unavailable', 'Alias target still requires a configured granted site');
    $config['publicAliases'] = ['https://old.invalid' => $origin];
    saveConfig();
    ...
    check(in_array(DestinationStore::peerTag($ref['instance']), $tags, true), 'Rendering registers peer dependency for connection-wide denial');
    ...
    due($ref);
    check($worker->run()['processed'] === 1, 'Due destination refreshes by identity over HTTPS');
    check($store->find($ref)['url'] === $origin . $slug . '-renamed', 'Renamed page updates the persistent canonical URL');
    check(!$pageCache->has('exchange_refresh_affected') && $pageCache->has('exchange_refresh_other'), 'Refresh invalidates dependent page cache only');
    check(str_contains($render(), $slug . '-renamed?utm_source=x%20y#heading'), 'Rendering uses renamed URL with unchanged per-link suffixes');
    $db->update('pages', ['hidden' => 1], ['uid' => $id]);
    $cache->flushCaches();
    due($ref);
    $worker->run();
    check($store->find($ref)['status'] === 'unavailable' && $render() === 'Read more', 'Confirmed hidden page becomes non-clickable');
    $db->update('pages', ['hidden' => 0], ['uid' => $id]);
    $cache->flushCaches();
    due($ref);
    $worker->run();
    check($store->find($ref)['status'] === 'resolved' && str_contains($render(), '<a'), 'Restored page becomes clickable again');
    ...
    due($ref);
    $worker->run();
    check($store->find($ref)['status'] === 'resolved', 'Page-tree move retains the verified identity');
    $db->update('pages', ['fe_group' => '99', 'extendToSubpages' => 1], ['uid' => $parent]);
    $cache->flushCaches();
    due($ref);
    $worker->run();
    check($store->find($ref)['status'] === 'unavailable', 'Inherited access restriction removes clickability');
    ...
    due($translatedRef);
    $worker->run();
    check($store->find($translatedRef)['status'] === 'unavailable', 'Missing selected translation never falls back during refresh');
    ...
    due($translatedRef);
    $worker->run();
    check($store->find($translatedRef)['url'] === $origin . '/de' . $slug . '-de', 'Newly available translation restores exact-language URL');
    $db->update('pages', ['slug' => $slug . '-de-renamed'], ['uid' => $translated]);
    $cache->flushCaches();
    due($translatedRef);
    $worker->run();
    check($store->find($translatedRef)['url'] === $origin . '/de' . $slug . '-de-renamed', 'Translated rename refreshes without changing selected language');
    ...
    foreach ([503, 429, 302] as $status) {
        ...
        due($ref);
        $worker->run();
        check($store->find($ref)['status'] === 'stale' && str_contains($render(), '-renamed'), 'HTTP ' . $status . ' retains the last verified URL');
    }
    $fake->status = 200;
    foreach (['timeout', 'wrong-identity', 'wrong-language', 'unsupported'] as $mode) {
        ...
        due($ref);
        $worker->run();
        check($store->find($ref)['status'] === 'stale' && str_contains($store->find($ref)['url'], '-renamed'), $mode . ' cannot retarget or remove a verified destination');
    }
    $fake->mode = 'resolved';
    $fake->suffix = '?unexpected=1';
    due($ref);
    $worker->run();
    check($store->find($ref)['status'] === 'stale', 'Refresh rejects per-link suffixes in canonical URLs');
    $fake->suffix = '';
    due($ref);
    $worker->run();
    check($store->find($ref)['status'] === 'resolved' && (int)$store->find($ref)['attempts'] === 0, 'Successful refresh resets backoff and stale state');
    $last = end($fake->calls);
    check($last['refs'] === [$ref] && $last['options']['verify'] === true && $last['options']['allow_redirects'] === false && $last['options']['timeout'] <= 3, 'Refresh uses explicit identity, verified TLS, no redirects, and bounded timeout');

    $fake->mode = 'unavailable';
    $fake->suffix = '?untrusted=1';
    due($ref);
    $worker->run();
    check($store->find($ref)['url'] === $origin . '/verified', 'Unavailable response cannot replace last verified URL with extra payload data');
    $fake->suffix = '';
    $fake->mode = 'timeout';
    due($ref);
    $worker->run();
    check($store->find($ref)['status'] === 'unavailable' && $render() === 'Read more', 'Outage never restores a confirmed unavailable link');
    check((int)$store->find($ref)['next_refresh'] > time(), 'Transient errors schedule exponential backoff');
    $fake->mode = 'resolved';
    $fake->onRequest = static fn() => $store->record($ref, 'resolved', $origin . '/newer-save');
    due($ref);
    $worker->run();
    check($store->find($ref)['url'] === $origin . '/newer-save', 'Late refresh cannot overwrite a newer save-time resolution');
    $fake->onRequest = null;
    due($ref);
    $claim = $store->claim($ref['instance'], 1);
    check(count($claim) === 1 && $store->claim($ref['instance'], 1) === [], 'Lease prevents duplicate concurrent refresh');
    ...
    check(count($replacement) === 1 && !$store->finish($claim[0], null, 'test'), 'Expired lease is reclaimable and superseded worker cannot finish');
    ...
    due($ref);
    $fake->calls = [];
    check($worker->run(51)['processed'] === 51 && count($fake->calls) === 2, 'Worker respects run limit and fifty-reference batch bound');
    check(count($fake->calls[0]['refs']) === 50 && count($fake->calls[1]['refs']) === 1, 'Batches contain explicit identities including language');
    $pageCache->set('exchange_refresh_peer', 'old output', [DestinationStore::peerTag($ref['instance'])]);
    $fake->status = 403;
    due($ref);
    $worker->run(1);
    check($store->find($extra)['status'] === 'denied' && $store->find($ref)['status'] === 'denied', 'Credential denial suppresses every destination for that peer beyond the current batch');
    check(!$pageCache->has('exchange_refresh_peer') && $render() === 'Read more', 'Connection denial invalidates peer-tagged output');
    check($worker->run()['processed'] === 0, 'Denied peer stays paused without repeated requests');
    $fake->status = 200;
    $worker->run(1, 'self');
    ...
    check($deniedCount === 52, 'Administrator resume restores only individually verified destinations');
    $worker->run();
    check($store->find($ref)['status'] === 'resolved', 'Resumed peer eventually verifies remaining destinations');
    due($ref);
    $fake->status = 401;
    $worker->run(1);
    ...
    $fake->status = 200;
    $worker->run();
    check($store->find($ref)['status'] === 'resolved' && end($fake->calls)['endpoint'] === $config['outgoing']['self']['endpoint'], 'Explicit credential and API endpoint correction resumes identity checks');
    check($store->find($ref)['url'] === 'https://new-public.invalid/verified', 'Explicit public domain update preserves managed identity');
    ...
    $cache->flushCaches();
    check(str_contains($render(), '/verified?utm_source=x%20y#heading'), 'Persistent refreshed data renders without HTTP after cache clear');
    check($contentDb->select(['header_link'], 'tt_content', ['uid' => $contentId])->fetchOne() === $managed, 'Refresh preserves the stored identity and per-link suffixes');
    echo "Passed $checks destination refresh checks.\n";
} finally {
    $cleanup();
}
